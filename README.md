...
2. Taper `pass` pour lister tous les secrets
...
1. Au chargement, le plugin exécute `gopass list --flat` pour récupérer tous les chemins de secrets
...
4. `executeItem` lance `gopass show -c <secret>` qui déchiffre et copie le mot de passe dans le presse-papier
5. Un rafraîchissement automatique se déclenche si le cache est vide ou périmé
